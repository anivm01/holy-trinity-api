...
exports.readAll = async (_req, res) => {
  try {
    const announcementData = await knex.select("*").from("weekly_announcement_bg");
    if (announcementData.length === 0) {
...
exports.updateSingle = async (req, res) => {
  try {
    ...
    const updatedAnnouncement = await knex("weekly_announcement_bg").select("*").where({
      id: req.params.id,
    });
...
exports.deleteSingle = async (req, res) => {
    try {
        const verify = await knex("weekly_announcement_bg").select("*").where({en_id: req.params.id})
        if (verify.length === 0) {
